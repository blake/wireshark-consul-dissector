...
-- Set up expert information
local yamux_expert_info = {
    -- errors
    unknown_packet_type = ProtoExpert.new("yamux.expert.unknown_packet_type", "Unknown packet type", expert.group.UNDECODED, expert.severity.ERROR),
    malformed_packet_type = ProtoExpert.new("yamux.expert.malformed_packet_type", "Malformed packet type", expert.group.MALFORMED, expert.severity.WARN),
    open_default_stream = ProtoExpert.new("yamux.expert.open_default_stream", "Default opened stream cannot be opened", expert.group.MALFORMED, expert.severity.WARN),
    data_before_start = ProtoExpert.new("yamux.expert.data_before_start", "Stream packets were detected without SYN flag", expert.group.SEQUENCE, expert.severity.ERROR),

    -- dissector helpful
    corrupted_frame = ProtoExpert.new("yamux.expert.protocol.corrupted_frame", "Frame is corrupted", expert.group.PROTOCOL, expert.severity.WARN),
    undecoded_direction = ProtoExpert.new("yamux.expert.protocol.undecoded_direction", "Cannot detect client-server direction. Addresses are equal", expert.group.PROTOCOL, expert.severity.NOTE),

    -- for user debugging
    client_packet = ProtoExpert.new("yamux.expert.packet_type.client", "Current packet is generated by client", expert.group.COMMENTS_GROUP, expert.severity.COMMENT),
    server_packet = ProtoExpert.new("yamux.expert.packet_type.server", "Current packet is generated by server", expert.group.COMMENTS_GROUP, expert.severity.COMMENT),
}

-- Add the defined fields to the protocol
proto_yamux.fields = yamux_fields
proto_yamux.experts = yamux_expert_info
...
local function parse_yamux(subtree, pinfo, fields, yamux_header)
    ...
    elseif typeValue == hdr_type.GO_AWAY then
        ...
        subtree:add_proto_expert_info(yamux_expert_info.unknown_packet_type, "Bad Yamux packet type " .. tostring(typeValue))
    ...
    if is_yamux_stream_default_created() and not (typeValue == hdr_type.PING or typeValue == hdr_type.GO_AWAY) then
        subtree:add_proto_expert_info(yamux_expert_info.malformed_packet_type, "Default opened stream cannot be with type " .. tostring(typeValue))
    ...
    if flags == HDR_FLAG_NAME_MAP.SYN then
        if get_src_addr() == get_dst_addr() then
            ...
            subtree:add_proto_expert_info(yamux_expert_info.undecoded_direction, "Default opened stream cannot be with type " .. tostring(typeValue))
        ...
        elseif is_yamux_stream_default_created() then
            disable_window_calc_logic = true
            subtree:add_proto_expert_info(yamux_expert_info.open_default_stream)
    ...
    if not (disable_window_calc_logic or is_yamux_stream_default_created()) then
        ...
        if previous_frame then
            if yamux_stream_info[current_stream_id].client_window_size and yamux_stream_info[current_stream_id].server_window_size then
                ...
                subtree:add_proto_expert_info(yamux_expert_info.corrupted_frame, "Previous frame is corrupted: no window size is available")
        ...
        if get_src_addr() == yamux_stream_info[current_stream_id].server_address then
            subtree:add_proto_expert_info(yamux_expert_info.server_packet)
        ...
        elseif get_dst_addr() == yamux_stream_info[current_stream_id].server_address then
            subtree:add_proto_expert_info(yamux_expert_info.client_packet)
            ...
            subtree:add_proto_expert_info(yamux_expert_info.data_before_start)
